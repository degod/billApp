...
export class ElectricPage {
	...
	fetchNets() {
		this.loading = this.loadingCtrl.create({
			content: 'Fetching DISCOs...'
		});
		this.loading.present();
		this.web.getElectric().then(
	...
	verifyBiller() {
		this.name = "";
		this.plan = (this.variation==false) ? "prepaid" : "postpaid";
		let id = this.network.split('||')[1];
		this.loading = this.loadingCtrl.create({
			content: 'Verifying Meter Number...'
		});
		this.loading.present();
		this.web.verifyBiller(id, this.plan, this.billersCode).then(
			(res2: any = {status: null, message: {content: null}}) => {
				this.loading.dismissAll();
				let ntes = JSON.parse(res2);
				console.log(ntes);

				if(ntes.message.code == '000'){
					this.name = ntes.message.content.Customer_Name;
				}
			}, err => {}
		);
	}
	...
	pay(){
		...
		let pln = (this.variation==false) ? "prepaid" : "postpaid";
		this.loading = this.loadingCtrl.create({
			content: 'Buying Data...'
		});
		this.loading.present();
		let cardTrk = "CRT-"+this.web.rand(100000,999999)+this.web.rand(100000,999999);
		const browser = this.iab.create(this.web.siteaddress2+'process_transaction.php?email='+this.email+'&phone='+this.phone+'&amount='+this.amount+'&billersCode='+this.billersCode+'&serviceID='+ntwk+'&variation='+pln+'&convenienceFee=100&type=tv', '_blank', { hardwareback: 'no', location: 'yes'});
