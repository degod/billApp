...
export class TvPage {
	...
	getBouquets() {
		...
		this.loading = this.loadingCtrl.create({
			content: 'Fetching Bouquets...'
		});
		this.loading.present();
		this.web.getBouquets(id).then(
			(res2: any = {status: null, message: {content: null}}) => {
				this.loading.dismissAll();
				this.plans = [];
				this.amount = [];
				...
				if(ntes.response_description == '000'){
					this.plans = ntes.content.varations;
				}
			}, err => {}
		);
	}
